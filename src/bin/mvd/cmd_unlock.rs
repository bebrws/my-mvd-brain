...
pub fn run(args: UnlockArgs) -> Result<()> {
    #[cfg(feature = "encryption")]
    {
        ...
        let result = memvid_core::encryption::unlock_file(&args.file, out, password.as_bytes())
            .map_err(|e| anyhow::anyhow!("{e}"))?;

        if args.json {
            println!("{{\"decrypted\":true,\"output\":\"{}\"}}", result.display());
        } else {
            println!("Decrypted {} → {}", args.file.display(), result.display());
        }
        Ok(())
    }
